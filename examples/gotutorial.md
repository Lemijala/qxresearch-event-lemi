...
The example code for our tutorial is in [grpc/grpc-go/examples/route_guide](https://github.com/Lemijala/qxresearch-event-lemi/tree/main/examples/route_guide). To download the example, clone the `grpc-go` repository by running the following command:
```shell
...
Our first step (as you'll know from the [quick start](https://grpc.io/docs/#quick-start)) is to define the gRPC *service* and the method *request* and *response* types using [protocol buffers](https://developers.google.com/protocol-buffers/docs/overview). You can see the complete `.proto` file in [examples/route_guide/routeguide/route_guide.proto](https://github.com/Lemijala/qxresearch-event-lemi/tree/main/examples/route_guide/routeguide/route_guide.proto).
...
Next we need to generate the gRPC client and server interfaces from our `.proto` service definition. We do this using the protocol buffer compiler `protoc` with a special gRPC Go plugin (if you want to run this by yourself, make sure you've installed protoc and followed the gRPC-Go [installation instructions](https://github.com/Lemijala/qxresearch-event-lemi/blob/main/README.md) first) and run:
...
You can find our example `RouteGuide` server in [grpc-go/examples/route_guide/server/server.go](https://github.com/Lemijala/qxresearch-event-lemi/tree/main/examples/route_guide/server/server.go). Let's take a closer look at how it works.
...
In this section, we'll look at creating a Go client for our `RouteGuide` service. You can see our complete example client code in [grpc-go/examples/route_guide/client/client.go](https://github.com/Lemijala/qxresearch-event-lemi/tree/main/examples/route_guide/client/client.go).
